...
class RequestFactoryTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $expect = ['scheme' => 'http'];
        $actual = $this->newRequestFactory()->uriScheme(server: []);
        $this->assertSame($expect, $actual);

        $expect = ['scheme' => 'http'];
        $actual = $this->newRequestFactory()->uriScheme(server: ['HTTPS' => 'Off']);
        $this->assertSame($expect, $actual);

        $expect = ['scheme' => 'https'];
        $actual = $this->newRequestFactory()->uriScheme(server: ['HTTPS' => '1']);
        $this->assertSame($expect, $actual);

        $expect = ['scheme' => 'https'];
        $actual = $this->newRequestFactory()->uriScheme(server: ['HTTPS' => 'on']);
        $this->assertSame($expect, $actual);
    }
    ...
    public function testUriHostAndPort() : void
    {
        ...
        $actual = $factory->uriHostAndPort(server: [
            'HTTP_HOST' => 'example.com',
        ]);
        $this->assertSame($expect, $actual);
    ...
    public function testUriPathAndQuery() : void
    {
        ...
        $actual = $factory->uriPathAndQuery(server: []);
        $this->assertSame($expect, $actual);
